...
import "./login.css";
import { loginWithCognito, completeNewPassword } from "../utils/cognitoAuth";
...
async function resolveUserFromToken(idToken, safeEmail) {
...
export function Login({ onLogin }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // First-time login — student must set their own password
      ...
      await finishLogin(result, safeEmail);
    } catch (err) {
  ...
  async function handleSetNewPassword(e) {
    ...
    try {
      const safeEmail = email.trim().toLowerCase();
      const result = await completeNewPassword(pendingCognitoUser, newPassword);
      await finishLogin(result, safeEmail);
    } catch (err) {
      handleLoginError(err);
    } finally {
  ...
  async function finishLogin(result, safeEmail) {
    const idToken = result.getIdToken().getJwtToken();
    const accessToken = result.getAccessToken().getJwtToken();
    const refreshToken = result.getRefreshToken().getToken();

    const user = await resolveUserFromToken(idToken, safeEmail);
  ...
  function handleLoginError(err) {
    ...
    } else if (message.includes("Password does not conform to policy")) {
    ...
    return (
      <div className="p17-page">
        ...
        <div className="p17-card">
          <h2 className="p17-card-title">Set your password</h2>
          <p className="p17-card-sub">Welcome! Please set a permanent password to activate your account.</p>

          <form className="p17-form" onSubmit={handleSetNewPassword}>
            ...
            <label className="p17-label">Confirm Password</label>
            <div className="p17-input-wrap">
              <span className="p17-left-icon" aria-hidden>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 1a5 5 0 0 0-5 5v3H6a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8a2 2 0 0 0-2-2h-1V6a5 5 0 0 0-5-5Zm3 8H9V6a3 3 0 0 1 6 0v3Z" />
