import { card, outlineBtn, scoreColor } from "../utils/styles";

export default function StudentDetail({ student, onBack }) {
  if (!student) {
    return (
      <div style={{ padding: "80px 16px", maxWidth: 960, margin: "0 auto" }}>
        <button onClick={onBack} style={outlineBtn({ fontSize: 12 })}>← back</button>
    ...
    <div style={{ padding: "80px 16px", maxWidth: 1000, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12 }}>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ fontWeight: 700, color: scoreColor(student.score), fontSize: 36, lineHeight: 1 }}>
            {Math.round(student.score)}
          </div>
          <div style={{ color: scoreColor(student.score), fontSize: 12, marginTop: -2 }}>%</div>
        </div>
      </div>
...
function MetricBar({ label, value }) {
  ...
  return (
    <li style={{ display: "grid", gridTemplateColumns: "160px 1fr 40px", alignItems: "center", gap: 10 }}>
      <span style={{ width: 160, color: "#6b7280", fontSize: 12 }}>{label}</span>
      <span style={{ flex: 1 }}>
        <div style={{ height: 8, borderRadius: 999, background: "#e5e7eb" }}>
          <div style={{ width: `${percentage}%`, height: "100%", borderRadius: 999, background: "#2563eb" }} />
        </div>
      </span>
