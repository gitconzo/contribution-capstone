...
export default function UploadFile({ darkMode }) {
  ...
  useEffect(() => {
    if (!uploadTeamId) { setSprints([]); return; }
    apiFetch(`/api/sprints/team/${uploadTeamId}`)
      .then(r => r.json())
        ...
        {sprints.length > 0 && (
          <div style={{ marginTop: 12 }}>
            ...
            <div style={{ fontSize: 12, color: theme.subtext, marginTop: 4 }}>
              Tag this document to a sprint so it's included in sprint-specific scoring.
            </div>
          </div>
        )}
                        ...
                        <td style={tableCellStyle}>
                          ...
                          {f.sprintId && (
                            <div style={{ fontSize: 11, marginTop: 3, color: "#0369a1", background: "#e0f2fe", padding: "1px 6px", borderRadius: 999, display: "inline-block" }}>
                              {sprints.find(s => s.id === f.sprintId)?.name || "Sprint"}
                            </div>
                          )}
                        </td>
                        <td style={tableCellStyle}>
