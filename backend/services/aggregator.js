...
function normalizeRepoUrl(url = "") {
  return String(url).trim().replace(/\.git$/i, "").replace(/\/+$/, "").toLowerCase();
}
...
async function aggregateForTeam(team, rootDir) {
  ...
  // Apply GitHub metrics only if the currently assigned repo matches the last analyzed repo
  const analyzedRepo = safeReadJson(path.join(dataDir, "repo.json"), null);
  const teamRepoUrl = normalizeRepoUrl(team.repo_url || "");
  const analyzedRepoUrl = normalizeRepoUrl(analyzedRepo?.url || "");

  const canUseGitHubMetrics =
    !!teamRepoUrl &&
    !!analyzedRepoUrl &&
    teamRepoUrl === analyzedRepoUrl;

  if (canUseGitHubMetrics) {
    ...
    Object.entries(githubMetrics).forEach(([author, metrics]) => {
      ...
      if (studentIndex === -1) {
        studentIndex = matchStudentIndex(
          aliasMap,
          author.replace(/\s*\[bot\]\s*$/i, "")
        );
      }
  ...
  const combinedDocsPath = path.join(
    rootDir,
    "data",
    "combined_documentation_metrics.json"
  );
  ...
  if (!fs.existsSync(combinedDocsPath)) {
    try {
      ...
      console.warn(
        "Could not generate combined_documentation_metrics.json:",
        err.message
      );
    }
  }
  ...
  parsedArtifacts.attendance.forEach((attendanceJson) => {
    ...
    Object.entries(byStudent).forEach(([key, attendanceData]) => {
      ...
      if (studentIndex !== -1) {
        students[studentIndex].attendance.percentage = pickNumber(
          attendanceData.attendance
        );
        students[studentIndex].attendance.meetings = pickNumber(
          attendanceData.meetings
        );
        students[studentIndex].attendance.hours += pickNumber(
  ...
  const combinedDocs =
    parsedArtifacts.sprint.length || parsedArtifacts.project.length
  ...
  if (combinedDocs?.students) {
    Object.entries(combinedDocs.students).forEach(([studentName, data]) => {
      ...
      if (studentIndex !== -1 && data.combined) {
        students[studentIndex].docs.docCount =
          data.combined.total_docs || 0;
        students[studentIndex].docs.words =
          data.combined.total_word_count || 0;
        students[studentIndex].docs.sections =
          data.combined.total_sections || 0;
        students[studentIndex].docs.avgSentenceLength =
          data.combined.avg_sentence_length || 0;
        students[studentIndex].docs.sentenceComplexity =
          data.combined.sentence_complexity || 0;
        students[studentIndex].docs.readability =
          data.combined.readability_score || 0;
      }
    });
  }
